...
bool UI_Button::update()
{
	...
	if (print)
	{
		...
		//{
		//	if (is_ui)
		//		App->view->LayerBlit(LAYER, App->gui->atlas, iPoint(rect.x, rect.y), curr,  -1.0f, false);
		//	else
	...
	if (print)
	{
		...
		for (list<tex_str>::iterator it = tex_str_list.begin(); it != tex_str_list.end(); it++)
		{
			if (!TextCmp((*it).text.c_str(), ""))
			{
				...
				{
					if(is_ui)
						App->view->LayerBlit(LAYER, (*it).texture, iPoint(rect.x, rect.y + space), { 0, 0, rect.w, rect.h }, -1.0f, false);
					else
...
bool UI_Image::update()
{
	...
	if (print)
	{
		...
		{
			if(is_ui)
				App->view->LayerBlit(LAYER, App->gui->atlas, iPoint(rect.x, rect.y), image, -1.0f, false);
			else
...
bool UI_Check_Box::update()
{
	...
	for (int i = 0; i < check_box_list.size(); i++)
	{
		...
		{
			if (is_ui)
				App->view->LayerBlit(LAYER, App->gui->atlas, iPoint(check_box_list.at(i)->button->rect.x, check_box_list.at(i)->button->rect.y), button, -1.0f, false);
			else
