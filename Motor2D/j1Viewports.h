...
struct layer_blit
{
	...
	layer_blit(SDL_Texture* _texture, iPoint _pos, const SDL_Rect _section, float _scale, bool _use_camera, SDL_RendererFlip _flip, double _angle, int _pivot_x, int _pivot_y)
	{
		texture = _texture; pos = _pos; section = {_section }; scale = _scale;
		flip = _flip; angle = _angle; pivot_x = _pivot_x; pivot_y = _pivot_y; use_camera = _use_camera;
	};
...
public:
	...
	void LayerBlit(int layer, SDL_Texture* texture, iPoint pos, const SDL_Rect section = NULLRECT, float scale = -1.0f, bool use_camera = true, SDL_RendererFlip _flip = SDL_FLIP_NONE, double angle = 0, int pivot_x = INT_MAX, int pivot_y = INT_MAX);
