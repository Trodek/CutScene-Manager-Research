...
bool MenuScene::Start()
{
	...
	// Start ---
	start_button = menu_window->CreateButton(iPoint(screen.w/2 - 70, 150), 223, 60, false);
	...
	start_text = menu_window->CreateText(iPoint(screen.w/2, 160), App->font->game_font);
	...
	// Options -
	options_button = menu_window->CreateButton(iPoint(screen.w/2 - 70, 220), 223, 60, false);
	...
	options_text = menu_window->CreateText(iPoint(screen.w/2, 230), App->font->game_font);
	options_text->SetText("OPTIONS");
	options_text->click_through = true;

	fx_button = menu_window->CreateButton(iPoint(screen.w/2 - 70, 220), 223, 60, false);
	...
	fx_text = menu_window->CreateText(iPoint(screen.w/2, 230), App->font->game_font);
	fx_text->SetText("FX");
	fx_text->enabled = false;

	music_button = menu_window->CreateButton(iPoint(screen.w/2 - 70, 290), 223, 60, false);
	...
	music_text = menu_window->CreateText(iPoint(screen.w/2, 300), App->font->game_font);
	...
	options_checkbox->AddBox(iPoint(screen.w/2 + 99, screen.h / 2 + 45), 44, 44, "fx");
	options_checkbox->AddBox(iPoint(screen.w/2 + 99, screen.h / 2 + 115), 44, 44, "music");
	...
	// Credits --
	credits_button = menu_window->CreateButton(iPoint(screen.w/2 - 70, 290), 223, 60, false);
	...
	credits_text = menu_window->CreateText(iPoint(screen.w/2, 300), App->font->game_font);
	credits_text->SetText("CREDITS"); 
	credits_text->click_through = true; 
	// ---------

	// Quit ---
	quit_button = menu_window->CreateButton(iPoint(screen.w/2 - 70, 360), 223, 60, false);
	...
	quit_text = menu_window->CreateText(iPoint(screen.w/2, 370), App->font->game_font);
...
bool MenuScene::Update(float dt)
{
	...
	if (App->input->GetKey(SDL_SCANCODE_UP) == KEY_DOWN && current_button > 0)
	{
		if (is_options)
		{
			if (current_button > 4)
...
bool MenuScene::PostUpdate()
{
	...
	if (App->input->GetKey(SDL_SCANCODE_BACKSPACE) == KEY_DOWN && is_options == true)
	{
		is_options = false; 
		GoMenu(); 
	}
